    ...
    <div class="sidebar flex-column flex-shrink-0 p-0 shadow-sm vh-100 sticky-top">
        ...
        <div class="p-4">
            <a href="/" class="d-flex align-items-center justify-content-center text-decoration-none">
                @if($societe && $societe->logo)
                    <img src="{{ asset('storage/' . $societe->logo) }}" alt="Logo" class="sidebar-logo-img rounded-3 shadow-sm w-100">
                @else
                    <div class="bg-primary rounded-3 p-2 me-2 shadow-sm d-flex align-items-center justify-content-center sidebar-fallback-logo-box">
        ...
        <div class="sidebar-content px-3 py-3 overflow-y-auto flex-grow-1">
            @if(auth()->user()->role === 'Admin')
                {{-- MENU ADMIN --}}
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">ADMINISTRATION</small>
            ...
            @elseif(auth()->user()->role === 'Agent')
                {{-- MENU AGENT --}}
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">VUE GÉNÉRALE</small>
                <ul class="nav nav-pills flex-column mb-4">
                    <li class="nav-item mb-1"><a href="{{ route('dashboard') }}"
                             class="nav-link d-flex align-items-center {{ request()->routeIs('dashboard') ? 'active' : '' }}"><i
                                 class="fas fa-th-large me-3"></i> Dashboard</a></li>
                </ul>
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">OPÉRATIONS SAV</small>
                <ul class="nav nav-pills flex-column mb-4">
                ...
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">CLIENTÈLE</small>
                <ul class="nav nav-pills flex-column mb-auto">
                    <li class="nav-item mb-1"><a href="{{ route('users.index') }}"
                             class="nav-link d-flex align-items-center {{ request()->routeIs('users.*') ? 'active' : '' }}"><i
            ...
            @elseif(auth()->user()->role === 'Technicien')
                {{-- MENU TECHNICIEN --}}
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">TABLEAU DE BORD</small>
                <ul class="nav nav-pills flex-column mb-4">
                    <li class="nav-item mb-1"><a href="{{ route('dashboard') }}"
                             class="nav-link d-flex align-items-center {{ request()->routeIs('dashboard') ? 'active' : '' }}"><i
                ...
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">ATELIER TECHNIQUE</small>
                <ul class="nav nav-pills flex-column mb-auto">
            ...
            @elseif(auth()->user()->role === 'Client')
                {{-- MENU CLIENT --}}
                <small class="text-muted text-uppercase fw-bold mb-2 d-block ps-3">MON ESPACE CLIENT</small>
                <ul class="nav nav-pills flex-column mb-auto">
